...
contract('測試SimpleCoin合約', async (accounts) => {
    ...
    it('應購買代幣', async function () {
        ...
        let buyAmount = new BigNumber(10**10);
        let receipt = 
        ...
        assert.equal(actualBalance.toNumber(), 
            previousBalance.toNumber() - totalSpend.toNumber(), '以太幣餘額不符');

        // 驗證代幣餘額
        let actualCoinBalance = await simpleCoin.balances(buyer1);
        assert.equal(actualCoinBalance.toNumber(), 
            buyAmount.toNumber(), '代幣餘額不符');
    });
    ...
    it('應購買代幣且回退函數成本更高', async function () {        
        ...
        console.log('呼叫一般函數的Gas用量: %d', gasUsedOfBuyCoin.toNumber());
        ...
        console.log('呼叫回退函數的Gas用量: %d', gasUsedOfFallbackFunction.toNumber());

        // 驗證兩次購買成本
        // !!! 注意: 這裡的成本使用一般函數還高於回退函數，與官方文件敘述不同，
        // 可能與測試環境實作方式有關，實際Gas耗用仍需以部署至真的以太坊的數值為準
        assert.isTrue(gasUsedOfBuyCoin.toNumber() > gasUsedOfFallbackFunction.toNumber(),
            '與欲期購買成本不符');

        // 驗證代幣餘額
        let actualCoinBalance = await simpleCoin.balances(buyer1);
        assert.equal(actualCoinBalance.toNumber(), 
            buyAmount.mul(2).toNumber(), '代幣餘額不符');
    });

    it('應造幣給buyer1', async function () {
        let previousCoinBalance = await simpleCoin.balances(buyer1);
        let mintAmount = new BigNumber(10**10);
        await simpleCoin.mintCoin(buyer1, mintAmount, {from: owner});
        let actualCoinBalance = await simpleCoin.balances(buyer1);
        assert.equal(actualCoinBalance.toNumber(), 
            previousCoinBalance.toNumber() + mintAmount.toNumber(), 
            '代幣餘額不符');
    });
    ...
    it('應無法造幣給buyer1', async function () {
        ...
        let actualCoinBalance = await simpleCoin.balances(buyer1);
        assert.equal(actualCoinBalance.toNumber(), 
            previousCoinBalance.toNumber(), '代幣餘額不符');
    });
    ...
    it('應燒毀buyer1的代幣', async function () {
        ...
        let mintAmount = new BigNumber(10**10);
        await simpleCoin.mintCoin(buyer1, mintAmount, {from: owner});
        let burnAmount = new BigNumber(10**9);
        await simpleCoin.burnCoin(buyer1, burnAmount, {from: owner});
        let actualCoinBalance = await simpleCoin.balances(buyer1);
        assert.equal(actualCoinBalance.toNumber(), previousCoinBalance.toNumber() 
            + mintAmount.toNumber() - burnAmount.toNumber(), '代幣餘額不符');
    });

    it('應轉出合約所有以太幣', async function(){
        // buyer1與buyer2先以以太幣購買代幣
        let buyAmount1 = new BigNumber(10**10);
        await simpleCoin.buyCoin({value: buyAmount1, from: buyer1});
        let buyAmount2 = new BigNumber(10**5);
        ...
        assert.equal(previousContractBalance.toNumber(), 
            buyAmount1.add(buyAmount2).toNumber(), '合約餘額不符');
        ...
        assert.equal(actualContractBalance.toNumber(), 0, '合約以太幣餘額不符');
        ...
        assert.equal(actualBalance.toNumber(), 
            previousBalance.sub(withdrawCost).add(withdrawAmount).toNumber(), 
            'owner以太幣餘額不符');
    });
})